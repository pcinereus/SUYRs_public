...
mckeon.rstanarm1 %>% ggpredict() %>% plot(add.data=TRUE, jitter=c(0.5,0))
```
...
mckeon.rstanarm1 %>% ggemmeans(~SYMBIONT) %>% plot(add.data=TRUE, jitter=c(0.5,0))
```
...
mckeon.rstanarm2 %>% ggemmeans(~SYMBIONT) %>% plot(add.data=TRUE, jitter=c(0.5,0))
```
...
priors <-
    ...
    ## prior(student_t(3,0,2), class = 'sd', coef = "Intercept", group = "BLOCK") +
    ## prior(student_t(3,0,2), class = 'sd') +
    ## prior(gamma(2,1), class = 'sd') +
    prior(gamma(2,1), class = 'sd') +
    ## prior(gamma(2,1), class = 'sd', coef = "Intercept", group = "BLOCK") +
...
wrap_elements(~testUniformity(mckeon.resids)) +
               ...
               wrap_elements(~plotResiduals(mckeon.resids)) +
