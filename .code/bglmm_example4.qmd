...
mckeon.sum <- summary(mckeon.brm4)
```
...
mckeon.tidy <- tidyMCMC(mckeon.brm4$fit, estimate.method='median',
                         conf.int=TRUE,  conf.method='HPDinterval',
                         rhat=TRUE, ess=TRUE)
```
...
mckeon.brm4 |> posterior_samples() |> as_tibble()
```
...
mckeon.brm4 |> bayes_R2(re.form=NA, summary=FALSE) |> median_hdci()
mckeon.brm4 |> bayes_R2(re.form=~(1|BLOCK), summary=FALSE) |> median_hdci()
