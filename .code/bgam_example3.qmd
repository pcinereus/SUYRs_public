...
wq.form <- bf(DOC ~ s(Dt.num), family = Gamma(link = "log"))
wq.sub |> summarise(mean(log(DOC)), sd(log(DOC)))
...
wq.brm3 |> conditional_effects() |>  plot(points=TRUE) |>
  _[[1]] +
  scale_y_log10()
wq.brm3 |> conditional_effects(spaghetti = TRUE) |>  plot(points=TRUE) |>
  _[[1]] +
  scale_y_log10()
```
...
wq.brm3 |> hypothesis('bs_sDt.num_1 = 0', class = '') |> plot()
wq.brm3 |> hypothesis('sds_sDt.num_1 = 0', class = '') |> plot()
wq.brm3 |> SUYR_prior_and_posterior()
```
::::
:::
<!-- END_PRIVATE-->
