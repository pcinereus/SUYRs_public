...
```{r fitModel2h_1a, results='markdown', eval=TRUE, mhidden=TRUE, cache=TRUE}
owls_form <- bf(NCalls ~ +
                    offset(log(BroodSize)) + (1|Nest),
                family=poisson(link='log'))
options(width=150)
...
```{r fitModel2h_1b, results='markdown', eval=TRUE, mhidden=TRUE, cache=TRUE}
...
owls_brm1b |>
  ...
  geom_point(data = owls, aes(y = NCalls/BroodSize),
    position = position_jitter(width = 0.2))
```
```{r posterior2h_1c, results='markdown', eval=TRUE, fig.width = 7, fig.height = 5}
...
```{r modelValidation6a_1, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=8}
