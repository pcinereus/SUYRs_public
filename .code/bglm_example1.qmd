...
Xmat = model.matrix(~FERTILIZER, data=newdata)
### get the posterior draws for the linear predictor
coefs <- fert.brm3 |> posterior_samples(pars=c('b_Intercept','b_scaleFERTILIZERscaleEQFALSE')) |> as.matrix()
fit <- coefs %*% t(Xmat)
fit |> median_hdci() |> bind_cols(newdata)
```
...
fert.brm3 |> hypothesis('scaleFERTILIZERscaleEQFALSE > 0') |> plot (ignore_prior= TRUE)
fert.brm3 |> hypothesis('scaleFERTILIZERscaleEQFALSE > 0') |> _[['samples']] |> median_hdci()
```
