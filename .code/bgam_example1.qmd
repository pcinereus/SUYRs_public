...
data_gam |> summarise(mean(y), sd(y))
priors <- prior(normal(5, 1.6),  class='Intercept') +
  prior(normal(0, 3), class='b') +
  prior(student_t(3, 0, 1.6),  class='sigma') +
  prior(student_t(3, 0, 40), class =  "sds")
...
data_gam.peak <-
  data_gam.brm3 |>
  add_epred_draws(newdata = newdata, object = _, ndraws = 1000) |>
  ungroup() |> 
  group_by(.draw) |> 
  mutate(diff = .epred - lag(.epred)) |>
  summarise(x = x[which.min(abs(diff))]) |>
  median_hdci(x, .width = 0.925)
data_gam.peak
