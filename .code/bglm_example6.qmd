...
quinn.rstanarm1 <- stan_glm(RECRUITS~fSEASON*DENSITY, data = quinn,
                            family = poisson(link = 'log'),
                            prior_PD = TRUE, 
                            refresh = 0,
                            chains = 3, iter = 5000, thin = 5, warmup = 2000)
...
quinn.rstanarm2 <- stan_glm(RECRUITS~fSEASON*DENSITY, data = quinn,
                            family = poisson(link = 'log'),
...
quinn.brmP |> hypothesis('fSEASONSummer<0') |> plot()
quinn.brmP |> hypothesis('DENSITYLow<0') |> plot()
quinn.brmP |> hypothesis('fSEASONSummer:DENSITYLow<0') |> plot()
```
...
quinn.rstanarmNB <- stan_glm(RECRUITS~fSEASON*DENSITY, data = quinn,
...
quinn.rstanarmNB |> ggpredict(~fSEASON+DENSITY) |> plot(show_data = TRUE)
```
...
quinn.brmsNB |> conditional_effects("fSEASON:DENSITY") |> plot(points = TRUE)
```

### ggpredict

```{r partialPlot2a, results='markdown', eval=TRUE, mhidden=TRUE, fig.width=8, fig.height=5}
quinn.brmsNB |> ggpredict(~fSEASON+DENSITY) |> plot(show_data = TRUE)
```
...
quinn.brmsNB |> ggemmeans(~fSEASON|DENSITY) |> plot(show_data = TRUE)
```
...
quinn.gather <- quinn.rstanarmNB |> gather_draws(`.Intercept.*|.*fSEASON.*|.*DENSITY.*`,  regex=TRUE) |>
  median_hdci()
```
...
quinn.gather <- quinn.brmsNB |> gather_draws(`.Intercept.*|.*fSEASON.*|.*DENSITY.*`,  regex = TRUE) |>
  median_hdci()
```
...
quinn.em |> group_by(contrast, fSEASON) |> median_hdci(Fit)
# Probability of effect
quinn.em |> group_by(contrast,fSEASON) |> summarize(P=mean(Fit>1))
##Probability of effect greater than 10%
quinn.em |> group_by(contrast,fSEASON) |> summarize(P=mean(Fit>1.1))
