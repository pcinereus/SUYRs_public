...
newdata <- with(loyn, data.frame(lAREA =  seq(min(log(AREA)), max(log(AREA)), len = 1000))) |>
...
```{r gbmstep1, eval=FALSE, mhidden=TRUE, cache=FALSE, mhidden=TRUE}
set.seed(123)
gbm_grid <- expand.grid(
  interaction.depth = c(1, 3, 5, 7),  # Depth of trees
  n.trees = c(5000, 10000),       # Number of trees
  shrinkage = c(0.01, 0.001),        # Learning rate
  n.minobsinnode = 2
)
cl<-makeCluster(1)
registerDoParallel(cl)
gbm_tuned <- train(
  ABUND ~ DIST+LDIST+AREA+fGRAZE+ALT+YR.ISOL,
  data = loyn,
  method = "gbm",
...
```{r gbmstep1a, eval=TRUE, mhidden=TRUE, cache=FALSE, mhidden=TRUE}
loyn.gbm1 <- gbm(ABUND ~ DIST+LDIST+AREA+fGRAZE+ALT+YR.ISOL,
                  data=loyn,
                  distribution='gaussian',
                  var.monotone=c(0,0,1,0,1,1),
                  n.trees=5000,
...
```{r gbmstep2, eval=FALSE, mhidden=TRUE, cache=FALSE}
summary(loyn.gbm1)  
```


```{r gbmstep3, eval=FALSE, mhidden=TRUE, cache=FALSE, fig.width=10,  fig.height=10}
gbm.plot(loyn.gbm1, n.plots=7, write.title = FALSE)  
```

```{r gbmstep4, eval=FALSE, mhidden=TRUE, cache=FALSE, fig.width=10,  fig.height=10}
gbm.plot.fits(loyn.gbm1)  
```


```{r gbmstep5, eval=FALSE, mhidden=TRUE, cache=FALSE, fig.width=10,  fig.height=10}
...
g1 + patchwork::inset_element(g2, left=0.7, bottom=0.01, right=1, top=0.4)
```
